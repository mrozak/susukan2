				...
				<form method="POST" enctype="multipart/form-data">
					...
					<div class="col-lg-12 mt-3">
						...
						<div class="col-lg-12 mt-1">
							...
							<a href="index.php?page=kebijakan" class="btn btn-danger btn-block">Batal</a>
						</div>
					</div>
				</form>
			</div>
	...
	if(isset($_POST['submit'])) {
		...
		if(!empty($gambar)) {
			if(in_array($ekstensi, $ekstensi_boleh) === true) {
				if($ukuran < 5000000) {
					move_uploaded_file($file_tmp, '../assets/file/pdf/kebijakan/'. $gambar);
					$sql = mysqli_query($con, "UPDATE tbl_kebijakan SET pdf='$gambar', judul='$judul', status='$status' WHERE id='$id'");
